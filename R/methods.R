#' Combine two time series in one
#'
...
#' @template author
#'
#' @param ...  The ts vectors to combine.
#' @return This function returns the ts vector.
#' @seealso \link[base]{c}
#' @keywords htest
#' @examples
#'
...
c.ts <- function(...){
    ellipsis <- list(...);
    # tsFrequencies <- sapply(ellipsis, frequency);
    # startTimes <- sapply(lapply(ellipsis, time), min);
    # tsOrders <- order(startTimes);
    # startTimes <- startTimes[tsOrders];
    # if(any(tsOrders!=c(1:length(tsOrders)))){
