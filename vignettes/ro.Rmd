...
Having the call, we need also to specify what the function should return. This can be the conditional mean (point forecasts), prediction intervals, the parameters of a model, or, in fact, anything that the model returns (e.g. name of the fitted model and its likelihood). The only requirement is that the value should be returned as a named element of a list and it should be a vector. If the function does not return a list, then you don't need to specify value at all - it will collect everything that is returned and give it back to you in a nice format. In order not to overcomplicate things, let's start with collecting the conditional mean from the `predict()` function:
```{r}
ourValue <- "pred"
```
...
The function returns a list with all the values that we asked for plus the actual values from the holdout sample. We can calculate some basic error measure based on those values, for example, scaled Mean Absolute Error (Petropoulos & Kourentzes, 2015):
```{r}
...
If we have a look at the returned matrices, we will notice that they contain NAs:
```{r}
...
As you see, `ro()` is convinient function, when you have one model and one time series. But what if you need to apply different models to different time series? We would need a loop, right? Yes. And there is a simple way to use `ro()` in this case. Let's continue our example but now introduce several time series:
```{r}
...
```
Although we do not specify `i` explicitly anywhere in the call of `ro()` above, it is used in `ourCall` and as a result the different models will be estimated in the inner loop. Comparing the performance of the two on the different time series we have (this is RelMAE from Davydenko & Fildes, 2013):
```{r}
...
```
Here we use `countf` variable, which specifies that we need to use both in-sample and the holdout (the full sample), as the function `es()` does not rely on `forecast()` or `predict()` and produces forecasts internally.
...
1. Davydenko A., Fildes R. (2013). Measuring forecasting accuracy: The case of judgmental adjustments to SKU-level demand forecasts. International Journal of Forecasting, 29, pp.510-522. DOI: [10.1016/j.ijforecast.2012.09.002](http://dx.doi.org/10.1016/j.ijforecast.2012.09.002).
2. Tashman L. (2000). Out-of-sample tests of forecasting accuracy: an analysis and review. International Journal of Forecasting, 16, pp.437-450. DOI: [10.1016/S0169-2070(00)00065-0](https://doi.org/10.1016/S0169-2070(00)00065-0).
3. Svetunkov I., Petropoulos F. (2017). Old dog, new tricks: a modelling view of simple moving averages. International Journal of Production Research, DOI: [10.1080/00207543.2017.1380326](https://dx.doi.org/10.1080/00207543.2017.1380326)
4. Petropoulos F., Kourentzes N. (2015). Forecast combinations for intermittent demand. Journal of the Operational Research Society, 66, pp.914-924. DOI: [10.1057/jors.2014.62](https://dx.doi.org/10.1057/jors.2014.62)
