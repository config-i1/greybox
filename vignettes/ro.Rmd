...
This is because by default the holdout sample is set to be non-constant: `co=FALSE` (constant out-sample = FALSE). The in-sample is also set to non-constant, that is why the model will be re-estimated on each iteration on the increasing sample. If we want to change that we might call `ro()` differently:
```{r}
