...
\end{equation}
where $b$ is the scale parameter, which, when estimated using likelihood, is equal to the mean absolute error:
...
\end{equation}
where $b$ is the scale parameter, $\alpha$ is skewness parameter and $I(y_t \leq \mu_t)$ is the indicator function, which is equal to one, when the condition is satisfied and to zero otherwise. The scale parameter $b$ estimated using likelihood is equal to the quantile loss:
...
There is currently onle one distribution in this group:
