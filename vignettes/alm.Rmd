...
2. [Density functions for continuous positive data](#continuousPlus),
3. [Continuous distributions on a specific interval](#continuousInterval),
4. [Density functions of discrete distributions](#discrete),
5. [Cumulative functions for binary variables](#cumulative).
...
The density of normal distribution $\mathrm{N}(\mu_t,\sigma)$ is:
...
\end{equation}
where $\epsilon_t \sim \mathrm{N}(0, \sigma^2)$.
...
\end{equation}
Thus maximising the likelihood \eqref{eq:ALaplace} is equivalent to estimating the linear regression \eqref{eq:linearModel} via the minimisation of $\alpha$ quantile, making this equivalent to quantile regression. So quantile regression models assume indirectly that the error term is $\epsilon_t \sim \mathrm{ALaplace}(0, s, \alpha)$ [@Geraci2007]. The advantage of using `alm()` in this case is in having the full distribution, which allows to do all the fancy things you can do when you have likelihood.
...
\end{equation}
Once again the maximisation of \eqref{eq:Logistic} implies the estimation of the linear model \eqref{eq:linearModel}, where $\epsilon_t \sim \mathrm{Logistic}(0, s)$.
...
Log Normal distribution appears when a normally distributed variable is exponentiated. This means that if $x \sim \mathrm{N}(\mu, \sigma^2)$, then $\exp x \sim \text{log}\mathrm{N}(\mu, \sigma^2)$. The density function of Log Normal distribution is:
...
\end{equation}
where $\epsilon_t \sim \mathrm{N}(0, \sigma^2)$ or:
...
Box-Cox Normal distribution used in the `greybox` package is defined as a distribution that becomes normal after the Box-Cox transformations. This means that if $x=\frac{y^\lambda+1}{\lambda}$, $x \sim \mathrm{N}(\mu, \sigma^2)$, then $y \sim \text{BC}\mathrm{N}(\mu, \sigma^2)$. The density function of the Box-Cox Normal distribution in this case is:
...
\end{equation}
and that $\epsilon_t \sim \mathrm{IG}(1, \phi)$. This means that $y_t \sim \mathrm{IG}\left(\mu_t, \frac{\phi}{\mu_t} \right)$, implying that the dispersion of the model changes together with the conditional expectation. The density function for the error term in this case is:
...
\begin{equation} \label{eq:Gamma}
    f(\epsilon_t) = \frac{1}{\Gamma(\sigma^{-2}) (\sigma^{2})^{\sigma^{-2}}} x^{\sigma^{-2}-1}\exp \left(-\frac{\epsilon_t}{\sigma^2}\right),
\end{equation}
...
\begin{equation} \label{eq:GammaDispersion}
    \hat{\phi} = \frac{1}{T} \sum_{t=1}^T \left(\epsilon_t - 1 \right)^2.
\end{equation}
...
`alm()` with `distribution="dgamma"` estimates the density for $y_t$ using `dgamma()` function from `stats` package. The $\mu_t$ is returned in the variables `mu` and `fitted.values`, the scale $\sigma^2$ is in the variable `scale`. `resid()` method returns $e_t = \frac{y_t}{\mu_t}$. Finally, the prediction and confidence intervals for the regression model are generated using `qgamma()` function from the `stats` package.
...
\begin{equation*}
    \log y_t = \mu_t + \epsilon_t,
\end{equation*}
where $\epsilon_t \sim \mathrm{Laplace}(0, \sigma^2)$. This distribution might be useful if the data has a strong skewness (larger than in case of log normal distribution).
...
\begin{equation*}
    \log y_t = \mu_t + \epsilon_t,
\end{equation*}
where $\epsilon_t \sim \mathrm{S}(0, \sigma^2)$. This distribution can be used for sever seldom right tail cases.
...
\begin{equation*}
    \log y_t = \mu_t + \epsilon_t,
\end{equation*}
where $\epsilon_t \sim \mathrm{GN}(0, s, \beta)$.
...
Folded Normal distribution is obtained when the absolute value of normally distributed variable is taken: if $x \sim \mathrm{N}(\mu, \sigma^2)$, then $|x| \sim \text{folded }\mathrm{N}(\mu, \sigma^2)$. The density function is:
...
\end{equation}
where $y\in (0,1)$, $y\sim \mathrm{logit}\mathrm{N}(\mu,\sigma^2)$ and $z\sim \mathrm{N}(\mu,\sigma^2)$. The bounds are not supported, because the variable $z$ becomes infinite. The density function of $y$ is:
