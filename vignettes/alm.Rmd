...
The model with Folded Normal distribution can be also summarised as:
\begin{equation} \label{eq:foldedNormalModel}
    y_t = \left| \mu_t + \epsilon_t \right| ,
\end{equation}
where $\epsilon_t$ is normally distributed error term.
...
The model with Chi Squared distribution can be also summarised as:
\begin{equation} \label{eq:chiSquaredModel}
    y_t = \left( \mu_t + \epsilon_t \right) ^2,
\end{equation}
where $\epsilon_t$ is normally distributed error term.

$\lambda_t$ is then returned in the variable `mu`, while $k$ is returned in `scale`. Finally, `fitted.values` returns $\lambda_t + k$. Similar correction is done in `predict()` function. As for the prediction intervals, they are generated using `qchisq()` function from `stats` package. Last but not least, `resid()` method returns $e_t = \log y_t - \log \mu_t$.
