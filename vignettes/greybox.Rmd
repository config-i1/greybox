...
2. `combine()` - combine the linear models into one using IC weights;
...
```{r BJcombine2}
ourModel <- combine(BJxreg,bruteForce=FALSE)
summary(ourModel)
```
...
ourModel <- combine(BJInsample,bruteForce=FALSE)
```
A plot of the model:
```{r BJcombinePlot}
plot(ourModel)
