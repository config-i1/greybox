...
2. `lmCombine()` - combine the linear models into one using IC weights;
3. `lmDynamic()` - produce model with dynamic weights and time varying parameters based on IC weight.
4. `xregExpander()` - expand the provided data by including leads and lags of the variables.
5. [ro()](ro.html) - produce forecasts with a specified function using rolling origin.
6. `rmc()` - regression for multiple comparison of methods. This is a parametric counterpart of `nemenyi()`, which should work better than `nemenyi()` on large samples.
