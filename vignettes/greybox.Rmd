...
3. [calm()](#calm) - combine the linear models into one using IC weights;
...
## Methods
The following methods can be applied to the models, produced by `alm()`, `stepwise()`, `calm()` and `lmDynamic()`:
...
2. `stick()` Seasonal, Trend, Irregular Contribution Kit based on Hans' Levenbach STI classification. Shows the proportion of the variability attributed to the specific time series components. A good starting point for the Exploratory Data Analysis.
...
```{r BJcombine2}
ourModel <- calm(BJxreg,bruteforce=FALSE)
summary(ourModel)
```
...
ourModel <- calm(BJInsample,bruteforce=FALSE)
```
